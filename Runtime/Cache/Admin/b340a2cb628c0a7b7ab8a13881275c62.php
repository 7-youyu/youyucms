    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">
                系统 <small>分类管理</small>
                <a href="<?php echo U('category/category_add');?>" class="btn btn-primary" title="add"  data-toggle="modal"  data-target="#myModal" data-target=".bs-example-modal-lg">新增</a>
            </eq>
            </h1>
            <div class="table-responsive back-main">
                <table class="table table-bordered table-hover table-striped">
                    ...
                    <tbody>
                        <?php if(is_array($category)): foreach($category as $key=>$list): ?><tr>
                            <td><?php echo ($list["id"]); ?></td>
                            <td><span class="label label-default"><?php echo ($list["name"]); ?></span></td>
                            ...
                            <?php if(is_array($list["child"])): foreach($list["child"] as $key=>$child): ?><tr>
                                <td><?php echo ($child["id"]); ?></td>
                                <td>&nbsp;┣&nbsp;&nbsp;<?php echo ($child["name"]); ?></td>
                                <td><?php echo ($child["url"]); ?></td>
                                <td><?php echo ($child["sort"]); ?></td>
                                <td><?php echo (get_status($child["status"])); ?></td>
                                <td>
                                    <div class="dropdown">
                                      ...
                                      <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">
                                        <li role="presentation"><a href="<?php echo U('category/category_edit',array('id'=>$child['id']));?>" data-toggle="modal" data-target="#myModal"><i class="icon-pencil" data-target=".bs-example-modal-lg"></i> 编辑</a></li>
                                        <li role="presentation"><a role="menuitem" tabindex="-1" href="<?php echo U('category/category_del',array('id'=>$child['id']));?>" onclick="winconfirm()"><i class="icon-trash"></i> 删除</a></li>
                                      </ul>
                                    </div>
                                </td>
