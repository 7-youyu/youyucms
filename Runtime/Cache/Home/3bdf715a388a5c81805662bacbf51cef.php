...
<html lang="zh-cn">
  ...
  <div class="main-full">
    <div class="page-header">
      <h3>项目成果 <small>Project Results</small></h3>
    </div>
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><div class="panel panel-default">
      <div class="panel-body">
        <div class="project-img <?php if(($mod) == "1"): ?>pull-right<?php else: ?>pull-left<?php endif; ?>">
          <a href="/my_project/lab_manage/youyulabmanage/3<?php echo (get_cover($list["cover_id"])); ?>" target="_blank"><img src="/my_project/lab_manage/youyulabmanage/3<?php echo (get_cover($list["cover_id"])); ?>" class="img-rounded img-responsive" alt=""></a>
        </div>
        <div class="project-text <?php if(($mod) == "1"): ?>pull-left<?php else: ?>pull-right<?php endif; ?>">
          <div class="caption">
            <h3><?php echo ($list["title"]); ?></h3>
            <p style="height:200px"><?php echo ($list["description"]); ?></p>
            <p><a href="<?php echo U('index/document',array('id'=>$list['id']));?>" class="btn btn-primary" role="button">详细</a> <?php if(!empty($list["show_url"])): ?><a href="<?php echo ($list["show_url"]); ?>" class="btn btn-default" role="button">演示地址</a><?php endif; ?></p>
          </div>
        </div>
        <div class="clear"></div>
      </div>
