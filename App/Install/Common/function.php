...
function check_dirfile(){
	$items = array(
		...
		array('file', '可写', 'ok', './App/Common/Conf/config.php'),
 ...
 */
function write_config($config, $email){
	if(is_array($config)){
		//读取配置内容
		...
		$conf = str_replace('[SUPER_ADMINI]', $email, $conf);
...
function register_administrator($db, $prefix, $admin, $auth){
	...
	$sql = "INSERT INTO `[PREFIX]user` VALUES ".
		   "(1, '[NAME]', '[EMAIL]', '[PASS]', 1, '0000-00-00', '', 0, 0, [IP], [TIME], [IP], [TIME], 0, 1);";
