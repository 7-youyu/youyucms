	...
	public function login($email, $password){
		...
		if(is_array($user) && $user['status']){
			...
			if(md5($password) === $user['password']){
				...
				$session_user             = array();		
				$session_user['uid']      = $user['uid'];
				$session_user['username'] = $user['username'];
				$session_user['email']    = $user['email'];
				session('user', $session_user);
				session('user_sign', data_auth_sign($session_user));

				setcookie("uid", $session_user['uid'], time()+3600*24);  
				setcookie("username", $session_user['username'], time()+3600*24);  
				setcookie("email", $session_user['email'], time()+3600*24);  
				// var_dump($_COOKIE);
